

[![AltSchool Badge](https://img.shields.io/badge/-Engineering-6773E5?style=for-the-badge&logo=data:image/png;base64,&logoColor=white&link=https://altschoolafrica.com/schools/engineering)](https://altschoolafrica.com/schools/engineering)

A mock pizza order/delivery API

## Assignment

- [U+1F4AF] add a user schema, the attributes/fields are up to you but the required fields/attributes are:
  - username,
  - password and
  - user_type.
  - user_type should be “admin” or “user”
- [U+1F4AF	] using JASONWEBTOKEN, protect the order routes

- [U+1F4AF] add query params to the /orders route, we want to:

  - sort the total_price from ascending to descending
  - sort the date created from asc to desc
  - query by a state

- [U+1F4AF] add pagination to the GET /orders route

**Bonus**

- [ ] Write integration tests for all the routes
- [U+1F4AF] Refactor the code such that we have routers for orders and users

<details>
<summary> Development </summary>

Run `npm install` to install dependencies used, then run `npm start dev` to start the devlopment server on `http://localhost:3334/`

</details>
