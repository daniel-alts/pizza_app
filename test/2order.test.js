...
describe('order route', () => {
    test('POST /order - first order by first user', async () => {
        let res = await supertest(server).post("/order").send(orders[0]).set("Authentication", `Bearer ${token1}`)
    ...
    test('POST /order - 2nd order by first user', async () => {
        let res = await supertest(server).post("/order").send(orders[1]).set("Authentication", `Bearer ${token1}`)
        expect(res.status).toBe(200)
        expect(res.body.status).toBe(true)
        expect(res.body.order.items[0].name).toBe("Pepperoni")
    ...
    test('POST /order - 3rd order by first user', async () => {
        let res = await supertest(server).post("/order").send(orders[2]).set("Authentication", `Bearer ${token1}`)
        expect(res.status).toBe(200)
        expect(res.body.status).toBe(true)
        expect(res.body.order.items[0].name).toBe("Pepperoni")
    ...
    test('GET /order - get all orders made by first user', async () => {
        let res = await supertest(server).get("/order?sort_by=total_price,created_at&order=asc&page=3&skip=0").set("Authentication", `Bearer ${token1}`)
        expect(res.status).toBe(200)
        expect(res.body.status).toBe(true)
    ...
    test('PATCH /order - update 3rd order made by 3rd user(admin) in db', async () => {
        let res = await supertest(server).patch("/order/3").send({state: 2}).set("Authentication", `Bearer ${token2}`)
        expect(res.status).toBe(200)
        expect(res.body.status).toBe(true)
    ...
    test('DELETE /order - delete order with id-3 made by 3rd user(admin) in db', async () => {
        let res = await supertest(server).delete("/order/3").set("Authentication", `Bearer ${token2}`)
        expect(res.status).toBe(200)
        expect(res.body.status).toBe(true)
    }, 70000);
})